...
def main():
    ...
    arg_parser.add_argument("--input", "-i", dest="file_in", type=str, help="Path to file with a url on each line to scrape and output to file.", required=True)
    ...
    if args.file_out != None:
        ...
        out = cv2.VideoWriter(args.file_out + ".mp4", cv2.VideoWriter_fourcc(*'mp4v'), cap.get(cv2.CAP_PROP_FPS), (new_width, new_height))
    ...
    while(cap.isOpened()):
        ...
        if args.file_out == None:
            ...
            # jump forward in video to simulate "real time"
            cap.set(cv2.CAP_PROP_POS_MSEC, cap.get(cv2.CAP_PROP_POS_MSEC) + processing_time)
        else:
